...
The repository includes GitHub Actions workflow templates so course projects can be validated and built directly from source-controlled repositories.
...
- uploads `ci-build-report.json`, `build-manifest.json`, and `build-summary.md`

Use this workflow to catch project-structure, schema, and graph errors before merge.
...
- uploads generated SCORM zips, per-build manifests, aggregate manifest, JSON report, and markdown summary
...
- uploads all generated SCORM zips plus the aggregate manifest and CI reports
